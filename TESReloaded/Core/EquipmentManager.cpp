...
public:
	...
	void TrackAttackHandling(UInt8 Arg1, TESObjectREFR* Arrow, TESObjectREFR* Target);
	UInt8 TrackProcessControlAttack();
	InventoryChanges::EntryData* TrackGetEquippedWeaponData(UInt8 Arg1);
	bool TrackSetEquippedWeaponData(InventoryChanges::EntryData* InventoryData, NiNode* ActorNode);
	...
	InventoryChanges::EntryData* TrackGetEquippedLightData(UInt8 Arg1);
	void TrackSaveGame();
	...
	if (SelectedRect) {
		...
		if (TheKeyboardManager->OnMouseUp(1)) {
			...
			if (SelectedRect->GetFloatValue(Tile::TileValue::kTileValue_user11, &fIndex)) {
				...
				if (InventoryData->type->typeID == TESForm::FormType::kFormType_Weapon) {
...
int Equipment::TrackProcessAction(float Arg1, float Arg2) {
	...
	if (Process && Process->GetProcessLevel() == 0 && Act->typeID == TESForm::FormType::kFormType_ACHR) {
		...
		if (LightData && Process->OnBackState == HighProcessEx::State::In && TheKeyboardManager->OnKeyDown(TheSettingManager->SettingsMain.EquipmentMode.TorchKey)) {
...
void Equipment::TrackUnequipItem(UInt8* Arg1, TESForm* Form, int Quantity, Actor* Act, ExtraDataList* DataList, UInt8 Left, UInt8 Lock) {
	...
	if (Process && Process->GetProcessLevel() == 0 && Form->typeID == TESForm::FormType::kFormType_Weapon && Left) Process->LeftEquippingState = HighProcessEx::State::Out;
	(this->*UnequipItem)(Arg1, Form, Quantity, Act, DataList, Left, Lock);
	if (Process && Process->GetProcessLevel() == 0 && Form->typeID == TESForm::FormType::kFormType_Weapon && Left) Process->LeftEquippingState = HighProcessEx::State::StateNone;
...
void Equipment::TrackSaveGame() {
	...
	if (EntryData->type->typeID == TESForm::FormType::kFormType_Weapon && EntryData->extendData) {
		for (TList<ExtraDataList>::Iterator Itr = EntryData->extendData->Begin(); !Itr.End() && Itr.Get(); ++Itr) {
			DataList = Itr.Get();
			if (DataList->HasType(BSExtraData::ExtraDataType::kExtraData_WornLeft)) {
...
{
	HighProcessEx* Process = (HighProcessEx*)Act->process;
	BSExtraData::ExtraDataType WornType = BSExtraData::ExtraDataType::kExtraData_Worn;
	
	IsWorn = 0;
	if (Process && Process->GetProcessLevel() == 0 && EntryData->type->typeID == TESForm::FormType::kFormType_Weapon) {
		if (Process->LeftEquippingState) WornType = BSExtraData::ExtraDataType::kExtraData_WornLeft;
		if (EntryData->extendData) {
			for (TList<ExtraDataList>::Iterator Itr = EntryData->extendData->Begin(); !Itr.End() && Itr.Get(); ++Itr) {
				if (Itr.Get()->HasType(WornType)) {
					IsWorn = 1;
					break;
				}
			}
...
void Prn(NiNode* ActorNode, NiNode* AttachToNode, NiNode* ObjectNode, SkinInfo* Info, int SkinInfoSlot) {
	...
	if (SkinInfoSlot == 13) {
		...
		if (Act->typeID == TESForm::FormType::kFormType_ACHR && Act->process->GetProcessLevel() == 0) {
